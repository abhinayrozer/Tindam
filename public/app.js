...
/* Google sign-in via Firebase Authentication: the SDK loads from Google's CDN
   only when the user clicks, then the popup's ID token is verified server-side. */
const GOOGLE_G = `<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true"><path fill="#FFC107" d="M43.6 20.1H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3l5.7-5.7C34.2 6.1 29.3 4 24 4 13 4 4 13 4 24s9 20 20 20 20-9 20-20c0-1.3-.1-2.6-.4-3.9z"/><path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.9 1.2 8 3l5.7-5.7C34.2 6.1 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.3 0-9.7-3.4-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z"/><path fill="#1976D2" d="M43.6 20.1H42V20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C36.9 39.2 44 34 44 24c0-1.3-.1-2.6-.4-3.9z"/></svg>`;

let firebaseAuthP = null;
function firebaseAuth() {
  if (!firebaseAuthP) {
    firebaseAuthP = (async () => {
      ...
      const appInst = initializeApp(state.firebaseConfig);
      return { auth: m.getAuth(appInst), m };
    })();
    firebaseAuthP.catch(() => { firebaseAuthP = null; });
  }
  return firebaseAuthP;
}

window.firebaseGoogle = async () => {
  ...
  try {
    const { auth, m } = await firebaseAuth();
    const provider = new m.GoogleAuthProvider();
    const cred = await m.signInWithPopup(auth, provider);
    const idToken = await cred.user.getIdToken();
    const res = await api('/api/auth/firebase', { idToken });
    afterLogin(res.user);
  } catch (e) {
    if (err && !/popup-closed|cancelled-popup/.test(String(e.code || ''))) {
      err.innerHTML = `<div class="error-box">${esc(e.message || 'Google sign-in failed')}</div>`;
    }
    mountGoogleButton(); // restore the button
  }
};
...
function mountGoogleButton() {
  ...
  if (state.firebaseConfig) {
    slot.innerHTML = `
      <button class="btn btn-google" id="fb-google-btn" onclick="firebaseGoogle()">${GOOGLE_G} Continue with Google</button>
      <div id="google-error"></div>`;
    return;
  }
...
(async function boot() {
  ...
  try {
    ...
    state.firebaseConfig = cfg.firebase || null;
