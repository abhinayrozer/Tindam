...
// Firebase ID tokens are RS256 JWTs signed by securetoken@system.gserviceaccount.com.
// Verified against Google's published x509 certs + audience/issuer/expiry.

let fbCertCache = { certs: null, fetchedAt: 0 };

function fetchFirebaseCerts() {
  return new Promise((resolve, reject) => {
    if (fbCertCache.certs && Date.now() - fbCertCache.fetchedAt < 6 * 3600 * 1000) {
      return resolve(fbCertCache.certs);
    }
    ...
    const req = https.get(url, { timeout: 6000 }, (res) => {
      ...
      res.on('end', () => {
        try {
          const certs = JSON.parse(raw); // { kid: pemCert }
          fbCertCache = { certs, fetchedAt: Date.now() };
          resolve(certs);
        } catch (e) { reject(e); }
      });
    });
    req.on('timeout', () => req.destroy(new Error('Firebase cert fetch timed out')));
    req.on('error', reject);
  });
}
...
async function verifyFirebaseIdToken(idToken, projectId) {
  ...
  } catch { throw new Error('Malformed Firebase ID token'); }

  if (header.alg !== 'RS256') throw new Error('Firebase token algorithm mismatch');
  ...
  if (!ok) throw new Error('Firebase token signature invalid');
  return { email: payload.email, name: payload.name || payload.email.split('@')[0], sub: payload.sub };
}
...
module.exports = {
  ...
  verifyGoogleIdToken, verifyFirebaseIdToken
};
